...
$(document).ready(function () {
    ...
    // *******************************start permissionm**************************
    permissionforlist('Maintenance_Log_List.aspx', 'CreateMaintenanceLog.aspx', 'btn_create');
    // *******************************end permissionm**************************
...
function GetAddressDetails() {
    Req = 'LogList';
    obj = "Fill";
    url = "Maintenance_Log_List.aspx/MaintenanceLogListDetails";
    ht = {};
...
function LoadAjaxAddress(ht, obj, Req, url) {
    ...
    $.ajax({
        ...
        success: function (Result) {
            ...
            if (obj == "Fill") {
                var data = jQuery.parseJSON(Result.d.LogData);
                var table = '<table id="AddressList" class="table table-bordered table-striped">';
                table = table + '<thead><tr><th style="display:none">ID</th><th>Unit</th><th>Maintenance Vendor</th><th>Amount</th><th>Repair Date</th><th  class=' + _allowedit + '>Edit</th><th  class=' + _allowdelete + '>Delete</th></tr></thead> <tbody>';
                $.each(data, function (i, item) {
                    ...
                    table = table + "<tr><td style='display:none' >" + item.ID +
                                    "</td><td>" + item.Unit +
                                    "</td><td>" + item.Maintenance_Vendor +
                                     "</td><td>" + item.Amount +
                                    "</td><td>" + item.Repair_Date +
                ...
                document.getElementById("MaintenanceLogListDiv").innerHTML = table + '</tbody></table>';
                setTimeout(function () {
            ...
            if (obj == "Delete") {
                ...
                if (Result.d.LogData != "" && Result.d.LogData != undefined) {
                    var json = jQuery.parseJSON(Result.d.LogData)[0];
...
function redirect() {
    window.location = 'CreateMaintenanceLog.aspx';
}
...
function AddressEdit(id) {
    ...
    window.location = 'CreateMaintenanceLog.aspx?' + bt;
}
