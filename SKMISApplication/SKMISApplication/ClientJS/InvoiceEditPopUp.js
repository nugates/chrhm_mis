        ...
        success: function (Result) {
            ...
            if (obj == "Country") {
                ...
                if (Result.d.Country != "" && Result.d.Country != undefined) {
                    ...
                    $('#cmbCountry_FC').html('');
                    $('#cmbCountry_FC').append($('<option></option>'));
                    $.each(Country, function (index, item) {
                        $('#cmbCountry_FC').append($('<option></option>').val(item.COUNTRY_ID).html(item.Name));
                    });
    ...
    $.ajax({
        ...
        success: function (Result) {
            ...
            if (obj == "BrokerContactSave") {
             ...
             if (obj == "FCSave") {
                 if (Result.d.FCSave != "" && Result.d.FCSave != undefined) {
                     var json = jQuery.parseJSON(Result.d.FCSave)[0];

                     if (json.CustomErrorState == "0") {

                         $("#btnCancel5_Cancel_FC").click();
                         $("#cmbFactoringCompany option[value='" + json.ID + "']").remove();
                         $('#cmbFactoringCompany').append($('<option></option>').val(json.ID).html($("#txtCompanyName_FC").val()));
                         $('#cmbFactoringCompany').val(json.ID).trigger('change');
             ...
             if (obj == "TermSave") {
                 if (Result.d.TermSave != "" && Result.d.TermSave != undefined) {
                     var json = jQuery.parseJSON(Result.d.TermSave)[0];

                     if (json.CustomErrorState == "0") {

                         $("#btnCancel5_Cancel_Term").click();
                         $("#cmbTerms option[value='" + json.ID + "']").remove();
                         $('#cmbTerms').append($('<option></option>').val(json.ID).html($("#txtTermName").val()));
                         $('#cmbTerms').val(json.ID).trigger('change');
...
$('#ShipperModal').on('hidden.bs.modal', function (e) {
    ...
    $("#txtAddressPhoneNumber_Shipper").val("");
});
...
$('#ConsigneeModal').on('hidden.bs.modal', function (e) {
    ...
    $("#txtAddressPhoneNumber_Consignee").val("");
});
...
//////////////////End Clear Popup////////////////////////////////////////////
///////////////// factoring Company add popup///////////////////////////////////

$("#Btn_AddFactoringCompany").click(function () {
    $("#btnsave5_Save_FC").text('Save');
    $("#FC_Heading").text('Add Factoring Company Details');
    $("#FCModal").modal({ backdrop: "static" });
    $("#FC_Address_Secton").show();
});


$("#Btn_UpdateFactoringCompany").click(function () {
    if ($('#cmbFactoringCompany').val() != "") {
        $("#btnsave5_Save_FC").text('Update');
        $("#FC_Heading").text('Update Factoring Company Details');
        $("#FCModal").modal({ backdrop: "static" });
        $("#FC_Address_Secton").hide();
        ...
        url = "InvoiceEdit.aspx/LoadDetails";
        ht = {};
        ht["ID"] = $("#cmbFactoringCompany").val();
        LoadAjaxLoad(ht, obj, Req, url);
    }
    else {
        swal({
            title: "Please Select a Factoring Company to Update",
            confirmButtonColor: "#5cb85c",
            confirmButtonText: "Ok!",
            closeOnConfirm: true,
            timer: 2000
...
$('#FCModal').on('hidden.bs.modal', function (e) {
    $("#txtCompanyName_FC").val("");
    $("#txtEIN_FC").val("");
    $("#txtContactName_FC").val("");
    $("#txtEmail_FC").val("");
    $("#txtDBAName_FC").val("");
    $("#txtAcNo_FC").val("");
    $("#cmbCountry_FC").val("").trigger('change');
    $("#cmbState_FC").val("").trigger('change');
    $("#txtAddress1_FC").val("");
    $("#txtSaveLocationAs_FC").val("");
    $("#txtCntactName_FC").val("");
    $("#txtCity_FC").val("");
    $("#txtAddress1_FC").val("");
    $("#txtZipCode_FC").val("");
    $("#txtAddress2_FC").val("");
    $("#txtInstructions_FC").val("");
    $("#txtPhoneNumber_FC").val(""); 
    $("#txtFactoringRate_FC").val("");
    $("#txtAddressPhoneNumber_FC").val("");
    $("#cmbFactoringRateUnit_FC").val("$").trigger('change');
});

function FCSave() {
    if (validationcheck_FC() == true) {
        setTimeout(function () {
            ht = {};

            ht["Company_Name"] = $("#txtCompanyName_FC").val();
            ht["EIN"] = $("#txtEIN_FC").val();
            ht["Contact_Name"] = $("#txtContactName_FC").val();
            ht["Email"] = $("#txtEmail_FC").val();
            ht["DBA_Name"] = $("#txtDBAName_FC").val();
            ht["Account_Number"] = $("#txtAcNo_FC").val();
            ht["Phone"] = $("#txtPhoneNumber_FC").val();
            ht["Factoring_Rate"] = $("#txtFactoringRate_FC").val();
            ht["Factoring_Rate_Unit"] = $("#cmbFactoringRateUnit_FC :selected").val();

            ht["Country"] = $("#cmbCountry_FC :selected").val();
            ht["State"] = $("#cmbState_FC :selected").val();
            ht["Address1"] = $("#txtAddress1_FC").val();
            ht["Save_Location_As"] = $("#txtSaveLocationAs_FC").val();
            ht["Address_Contact_Name"] = $("#txtCntactName_FC").val();
            ht["City"] = $("#txtCity_FC").val();
            ht["Zip_Code"] = $("#txtZipCode_FC").val();
            ht["Address2"] = $("#txtAddress2_FC").val();
            ht["Instructions"] = $("#txtInstructions_FC").val();
            ht["ContactPhoneNo"] = $("#txtAddressPhoneNumber_FC").val();


            if ($("#btnsave5_Save_FC").text() == "Save") {
                ht["ID"] = '';
                ht["MODE"] = "INSERT";
            }
            if ($("#btnsave5_Save_FC").text() == "Update") {
                ht["ID"] = $("#cmbFactoringCompany").val();
                ht["MODE"] = "UPDATE";
            }
...
function validationcheck_FC() {
    if ($('#txtCompanyName_FC').val() == "" || $('#txtCompanyName_FC').val() == null) {
        popupErrorMsg($("#txtCompanyName_FC"), "Company Name is Required.", 5);

        return false;
    }

    if ($('#txtFactoringRate_FC').val() == "" || $('#txtFactoringRate_FC').val() == null) {
        popupErrorMsg($("#txtFactoringRate_FC"), "Factoring Rate is Required.", 5);

        return false;
    }
    return true;
}
/////////////////End factoring Company add popup///////////////////////////////////

///////////////// Term add popup///////////////////////////////////

$("#Btn_AddTerms").click(function () {
    $("#btnsave5_Save_Term").text('Save');
    $("#Term_Heading").text('ADD INVOICE PAYMENT TERMS');
    $("#TermModal").modal({ backdrop: "static" });
});


$("#Btn_UpdateTerms").click(function () {
    if ($('#cmbTerms').val() != "") {
        $("#btnsave5_Save_Term").text('Update');
        $("#Term_Heading").text('UPDATE INVOICE PAYMENT TERMS');
        $("#TermModal").modal({ backdrop: "static" });
        ...
        swal({
            title: "Please Select a Term to Update",
            confirmButtonColor: "#5cb85c",
            confirmButtonText: "Ok!",
            closeOnConfirm: true,
            timer: 2000
...
{
    $('#txtTermName').val('NET ' + $('#txtNoofdays').val());
}
...
function TermSave() {
    if (validationcheck_Term() == true) {
        setTimeout(function () {
            ht = {};

            ht["NoofDays"] = $("#txtNoofdays").val();
            ht["Terms_Name"] = $("#txtTermName").val();

            if ($("#btnsave5_Save_Term").text() == "Save") {
                ht["ID"] = '';
                ht["MODE"] = "INSERT";
            }
            if ($("#btnsave5_Save_Term").text() == "Update") {
                ht["ID"] = $("#cmbTerms").val();
                ht["MODE"] = "UPDATE";
            }
...
function validationcheck_Term() {
    if ($('#txtNoofdays').val() == "" || $('#txtNoofdays').val() == null) {
        popupErrorMsg($("#txtNoofdays"), "No. of Days is Required.", 5);

        return false;
    }

    if ($('#txtTermName').val() == "" || $('#txtTermName').val() == null) {
        popupErrorMsg($("#txtTermName"), "Term Name is Required.", 5);
