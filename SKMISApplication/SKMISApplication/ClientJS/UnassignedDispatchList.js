...
function GetLoadDetails() {
    Req = 'LoadList';
    obj = "Fill";
    url = "UnassignedDispatchList.aspx/AllLoadDetails";
    ht = {};
    ...
    $.ajax({
        ...
        success: function (Result) {
            ...
            if (obj == "Fill") {
                ...
                table = table + '<thead><tr><th style="display:none">Load ID</th><th>Dispatch Number</th><th>Type</th><th>Name</th><th>Dispatch Date</th><th>Dispatch Time</th><th>Reference</th><th>Delivery Status</th><th>Cancel Status</th><th>Status</th><th  class=' + _allowedit + '>Edit</th><th  class=' + _allowdelete + '>Delete</th></tr></thead> <tbody>';
                $.each(data, function (i, item) {
                    if (item.IS_COMPLETE == "False") {
                        Complete_Status = 1
                        statusclass_complete = 'btn btn-block btn-info';
                        Text_complete = 'Not Delivered';
                    } else {
                        Complete_Status = 0
                        statusclass_complete = 'btn btn-block btn-warning';
                        Text_complete = 'Delivered';
                    }
                    ...
                    table = table + "<tr><td style='display:none' >" + item.ID +
                                    "</td><td>" + item.Dispatch_Number +
                                     "</td><td>" + item.IsCustCarrORBro +
                                    "</td><td>" + item.Name +
                                     "</td><td>" + item.Dispatch_Date +
                                      "</td><td>" + item.Dispatch_Time +
                                    ...
                                    "</td><td><input id=status_" + item.ID + " data=" + item.IS_CANCEL + " class='" + statusclass_cancel + "'  onclick=setComplete_cancel(" + Cancel_Status + "," + item.ID + ",'Cancel') type='button' value='" + Text_cancel + "' />" +
                                     "</td><td><input id=status_" + item.ID + " class='btn btn-block btn-primary'  onclick=LoadEdit(" + item.ID + ") type='button' value='Assign' />" +
...
function redirect() {
    window.location = 'CreateDispatch.aspx';
}
