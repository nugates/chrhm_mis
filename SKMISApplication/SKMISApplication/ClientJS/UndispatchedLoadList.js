...
function GetLoadDetails() {
    Req = 'LoadList';
    obj = "Fill";
    url = "UndispatchedLoadList.aspx/AllLoadDetails";
    ht = {};
    ...
    $.ajax({
        ...
        success: function (Result) {
            ...
            if (obj == "Fill") {
                ...
                table = table + '<thead><tr><th style="display:none">Load ID</th><th>Load Number</th><th>Load Type</th><th>Type</th><th>Name</th><th>Load Date</th><th>Load Time</th><th>Reference</th><th>Complete Status</th><th>Cancel Status</th><th>Dispatch Status</th><th  class=' + _allowedit + '>Edit</th><th  class=' + _allowdelete + '>Delete</th></tr></thead> <tbody>';
                $.each(data, function (i, item) {
                    ...
                    table = table + "<tr><td style='display:none' >" + item.ID +
                                    ...
                                    "</td><td><input id=status_" + item.ID + " data=" + item.IS_CANCEL + " class='" + statusclass_cancel + "'  onclick=setComplete_cancel(" + Cancel_Status + "," + item.ID + ",'Cancel') type='button' value='" + Text_cancel + "' />" +
                                     "</td><td><input id=status_" + item.ID + " class='btn btn-block btn-primary'  onclick=Create_Dispatch(" + item.ID + ") type='button' value='Create Dispatch' />" +
            ...
            if (obj == "Create_Dispatch") {
                if (Result.d.Create_Dispatch != "" && Result.d.Create_Dispatch != undefined) {
                    var json = jQuery.parseJSON(Result.d.Create_Dispatch)[0];

                    if (json.CustomErrorState == "0") {
                       var bt = btoa("Did=" + json.ID + "");
                       window.location = 'CreateDispatch.aspx?' + bt;
                    }
                    else if (json.CustomErrorState == "1") {
                        swal("", "Something went wrong , please try again later !!", "error");
